...
class BookingsTableViewCell: UITableViewCell {
  ...
  func configure(viewModel: BookingCellViewModel) {
  ...
  @IBAction func paymentButtonDidTapped(_ sender: Any) {
    delegate?.paymentButtonDidTapped()
    print("AA")
  }
  
  func hidePaymentButton() {
    paymentButton.isHidden = true
    paymentButtonHeightConstraint.constant = 0
    buttonContainerHeightConstraint.constant = 0
  }
  
  func unHidePaymentButton() {
    paymentButton.isHidden = false
    paymentButtonHeightConstraint.constant = 30
